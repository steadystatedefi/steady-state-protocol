...
interface IProtocolPayIn {
    ...
    function deposit(address /* IProtocol */ forProtocol, TokenAmount[] calldata amounts) external;

    /// @dev withdraws tokens from protocol's deposits.
    /// @dev only users allowed by IProtocol.hasRole(WITHDRAW) can do this
    function withdraw(address /* IProtocol */ forProtocol, TokenAmount[] calldata amounts) external;

    /// @dev returns amounts that were not yet consumed/locked by the stream of premium
    function balanceOf(address /* IProtocol protocol */) external view returns (TokenAmount[] memory);
    ...
    function coverageOf(address protocol) external view returns (uint256 requestedCoverage, uint256 providedCoverage, address token);
