...
import { NameTags, TracerDependenciesExtended } from 'hardhat-tracer/dist/src/types';
import { HardhatRuntimeEnvironment } from 'hardhat/types';

import { DRE } from './dre';
import { TNetwork, isAutoGasNetwork, TEthereumAddress } from './types';

export const isForkNetwork = (): boolean => !!process.env.FORK;

export const getNetworkName = (x?: string | HardhatRuntimeEnvironment): TNetwork => {
  const { FORK } = process.env;
  if (FORK) {
    return <TNetwork>FORK;
  }
  if (typeof x === 'string') {
    return <TNetwork>x;
  }
  if (x === undefined) {
    return <TNetwork>DRE.network.name;
  }
  return <TNetwork>x.network.name;
};
...
type HRE = HardhatRuntimeEnvironment & { tracer: TracerDependenciesExtended };

export const nameTags = (): NameTags => (DRE as HRE).tracer.nameTags;
